...
const ERRORS = {
    MISSING_SKOPEO:
        "/usr/bin/skopeo is not found in PATH. Are you using a container with skopeo installed?",
    ...
    IMAGE_EXISTS: (path) =>
        `Image was already found at ${path} and FORCE is deactivated.`,
...
async function verifyConditions(pluginConfig, context) {
    ...
    if (output.errors.length > 0) {
        ...
        throw new SemanticReleaseError(
            ERRORS.WRONG_SOURCE(output.errors.join(", ")),
            "EWRONG_SOURCE",
            output,
        );
    }
    ...
    if (!exists) {
        logger.info("Source does not exists (skopeo inspect).");
        throw new SemanticReleaseError(
            ERRORS.WRONG_SOURCE(config.source),
            "EWRONG_SOURCE",
            output,
        );
    }
    ...
    if (!config.destination) {
        ...
        throw new SemanticReleaseError(
            ERRORS.MISSING_DESTINATION,
            "EMISSING_DESTINATION",
        );
    }
    if (!Array.isArray(config.destination)) {
        logger.info("Destination is not a list.");
        throw new SemanticReleaseError(
            ERRORS.WRONG_DESTINATION("Destination must be a list of strings."),
            "EWRONG_DESTINATION",
        );
    }
    for (const destination of config.destination) {
        ...
        if (output.errors.length > 0) {
            ...
            throw new SemanticReleaseError(
                ERRORS.WRONG_DESTINATION(output.errors.join(", ")),
                "EWRONG_DESTINATION",
                output,
            );
        }
...
async function checkDestinationIsWriteable(destination, config) {
    ...
    if (exists && !config.force) {
        throw new SemanticReleaseError(
            ERRORS.IMAGE_EXISTS(destination),
            "EIMAGEEXISTS",
        );
    }
}
