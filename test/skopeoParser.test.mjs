...
describe("parseSkopeoDestination", () => {
    ...
    it("detects forbidden characters in details", () => {
        ...
        expect(result.errors).toContain(
            "Details contain forbidden characters: //registry.example.com/repo:bad tag!",
        );
    });
    ...
    it("detects uppercase characters in details", () => {
        ...
        const result = parseSkopeoDestination(input);
        expect(result.errors).toContain(
            "Details contain forbidden characters: //ghcr.io/LukasWestholt/semantic-release-skopeo/semantic-release-skopeo:latest",
        );
    });
    ...
    it("detects non-existing path for local targets", () => {
        ...
        const result = parseSkopeoDestination(input);
        expect(result.errors).toContain(
            "File does not exist: '/usr/src/app/relative/path'",
        );
    });
    ...
    it("detects missing colon in destination", () => {
        ...
        const result = parseSkopeoDestination(input);
        expect(result.errors).toContain(
            "Invalid format: missing ':' to separate transport and details.",
        );
    });
});
