...
describe('Comprehensive Test with All Arguments', () => {
    ...
    function getNewDestinations() {
        return [
            'docker://mock-registry:5000/my-project' +
                Date.now().toString() +
                '/my-image:${version}',
            'docker://mock-registry:5000/my-project' + Date.now().toString() + '/my-image:latest',
        ];
    }
    ...
    beforeEach(() => {
        ...
        pluginConfig = {
            copyArgs: ['--additional-tag=test:latest', '--dest-tls-verify=false'],
    ...
    it('should execute publish with all configuration parameters successfully even without force', async () => {
        ...
        expect(await checkImageExists(pluginConfigNew.destination[1], config)).toBeFalsy();
        await expect(publish(pluginConfigNew, context)).resolves.not.toThrow();
        expect(await checkImageExists(pluginConfigNew.destination[1], config)).toBeTruthy();
